    ...
    methods (Static)
        ...
        %{
            Purpose: generate a plot for the test configuration slopes and
                chirp cycle times
            Inputs: 
                slopes_MHz_us - the slope for each test
                chirp_cycle_times_MHz_us - the chirp cycle time for each
                    test
        %}
        function plot_test_configurations(slopes_MHz_us,chirp_cycle_times_MHz_us)
            figure;
        ...
        %{
            ...
            Inputs: 
                ...
                chirp_cycle_times_MHz_us - the chirp cycle time for each
                    test
        %}
        function plot_test_configuration_cdfs(slopes_MHz_us,chirp_cycle_times_MHz_us)
            figure;
            %plot cdf for slopes
            subplot(1,2,1);
...
%% Functions to Run Sensing Subsystem Simulations
        ...
        %{
            ...
            Inputs:
                ...
                save_file_name - path to save the results file to
        %}
        function testing_data = run_sensing_performance_test_cases( ...
                num_cases, ...
                frames_to_compute, ...
                slopes_MHz_us, ...
            ...
            testing_data = zeros(num_cases,size(save_file_headers,2));
            ...
            for i = 1:num_cases
                ...
                %save values
                testing_data(i,1) = actual_slope;
                testing_data(i,2) = actual_chirp_duration;
                testing_data(i,3) = actual_frame_duration;
                testing_data(i,4) = adc_sampling_rates(i);
                testing_data(i,5) = estimated_slope;
                testing_data(i,6) = estimated_chirp_duration;
                testing_data(i,7) = estimated_frame_duration;
                testing_data(i,8) = abs(actual_slope - estimated_slope);
                testing_data(i,9) = abs(actual_chirp_duration - estimated_chirp_duration);
                testing_data(i,10) = abs(actual_frame_duration - estimated_frame_duration);
            end

            %save the results to a file
            test_data_results = array2table(testing_data,"VariableNames",save_file_headers);
            writetable(test_data_results,save_file_name,'WriteRowNames',true); 
        end
    end
end